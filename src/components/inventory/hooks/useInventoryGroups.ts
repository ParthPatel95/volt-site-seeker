...
export function useInventoryGroups(workspaceId: string | null) {
  ...
  const {
    ...
    queryKey: ['inventory-groups', workspaceId],
    queryFn: async () => {
  ...
  const createGroupMutation = useMutation({
    mutationFn: async (group: Omit<InventoryGroup, 'id' | 'created_at' | 'updated_at' | 'group_code'>) => {
      ...
      // Build insert object with explicit fields - need project_id for DB constraint
      const insertData: Record<string, unknown> = {
        ...
        created_by: user?.id,
        // Note: project_id is still required in DB until we migrate, using workspace_id as placeholder
        project_id: group.workspace_id,
      };
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-groups', workspaceId] });

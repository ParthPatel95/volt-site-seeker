...
import { Camera, Keyboard, Loader2, Usb, ScanBarcode, CheckCircle2, X } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent 
        className={cn(
          "p-0",
          isMobile 
            ? "h-[100dvh] w-full max-w-full rounded-none border-0 sm:rounded-lg sm:border" 
            : "sm:max-w-md"
        )}
      >
        {/* Mobile Close Button */}
        {isMobile && (
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 right-4 z-50 h-10 w-10 rounded-full bg-background/80 backdrop-blur-sm"
            onClick={handleClose}
          >
            <X className="h-5 w-5" />
          </Button>
        ...
        <div className={cn("flex flex-col", isMobile && "h-full")}>
          ...
          <Tabs 
            value={activeTab} 
            onValueChange={(v) => setActiveTab(v as 'camera' | 'manual' | 'hardware')}
          ...
          >
            <div className="px-6">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="camera" className="flex items-center gap-2">
                  ...
                  <span className={isMobile ? "sr-only sm:not-sr-only" : ""}>Camera</span>
                </TabsTrigger>
                <TabsTrigger value="hardware" className="flex items-center gap-2">
                  <Usb className="w-4 h-4" />
                  <span className={isMobile ? "sr-only sm:not-sr-only" : ""}>USB</span>
                </TabsTrigger>
                <TabsTrigger value="manual" className="flex items-center gap-2">
                  <Keyboard className="w-4 h-4" />
                  <span className={isMobile ? "sr-only sm:not-sr-only" : ""}>Manual</span>
                </TabsTrigger>
              </TabsList>
            </div>
            ...
            >
              <div className={cn(
                ...
                <div className="absolute inset-0 pointer-events-none">
                  ...
                  <div className={cn(
                    "absolute border-2 border-primary rounded-lg",
                    isMobile ? "inset-[15%]" : "inset-[20%]"
                  )}>
            ...
            <TabsContent value="hardware" className="mt-4 px-6 pb-6">
            ...
            <TabsContent value="manual" className="mt-4 px-6 pb-6">

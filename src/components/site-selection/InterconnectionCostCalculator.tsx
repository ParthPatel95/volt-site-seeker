...
const InterconnectionCostCalculator = () => {
  ...
  const calculateCosts = () => {
    ...
    return { transmissionLine, substationTap, customerSubstation, engineeringStudies, utilityUpgrades, permitting, total };
  };
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div>
            ...
            <input type="range" min="0.5" max="10" step="0.5" value={inputs.distanceToSubstation}
            ...
            <div className="flex justify-between text-sm text-white/50 mt-1">
              <span>0.5 mi</span>
              <span className="text-secondary font-bold">{inputs.distanceToSubstation} miles</span>
              <span>10 mi</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {Object.keys(voltageMultipliers).map((voltage) => (
                <button key={voltage} onClick={() => setInputs(p => ({ ...p, voltageLevel: voltage }))}
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                    inputs.voltageLevel === voltage ? 'bg-primary text-primary-foreground' : 'bg-white/10 text-white/70 hover:bg-white/20'
                  }`}>
                  {voltage}
                </button>
              ))}
          ...
          <div>
            ...
            <input type="range" min="10" max="200" step="10" value={inputs.requiredMW}
              onChange={(e) => setInputs(p => ({ ...p, requiredMW: Number(e.target.value) }))}
              className="w-full h-2 bg-white/20 rounded-lg appearance-none cursor-pointer" />
            <div className="flex justify-between text-sm text-white/50 mt-1">
              <span>10 MW</span>
              <span className="text-secondary font-bold">{inputs.requiredMW} MW</span>
              <span>200 MW</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[{ key: 'none', label: 'None', desc: 'Existing capacity' }, { key: 'minor', label: 'Minor', desc: '+$2M' },
                { key: 'moderate', label: 'Moderate', desc: '+$8M' }, { key: 'major', label: 'Major', desc: '+$20M' }
              ].map((option) => (
                <button key={option.key} onClick={() => setInputs(p => ({ ...p, upgradeNeeded: option.key }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all ${
                    inputs.upgradeNeeded === option.key ? 'bg-primary text-primary-foreground' : 'bg-white/10 text-white/70 hover:bg-white/20'
                  }`}>
        ...
        <div>
          <div className="space-y-3 mb-6">
            {costBreakdown.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-white/10 rounded-lg">
                ...
                <span className={`font-bold ${item.value === 0 ? 'text-white/50' : 'text-secondary'}`}>
          ...
          <div className="p-4 bg-market-positive/20 rounded-xl text-center">
            <div className="text-sm text-white/70 mb-1">Estimated Total Cost</div>
            <div className="text-3xl font-bold text-market-positive">{formatCurrency(costs.total)}</div>
            <div className="text-xs text-white/50 mt-2">{inputs.requiredMW} MW @ {inputs.distanceToSubstation} miles</div>
          </div>

          <div className="mt-4 p-3 bg-secondary/20 rounded-lg">
            <p className="text-xs text-white/70">
              <strong className="text-secondary">⚠️ Note:</strong> Actual costs vary significantly based on 

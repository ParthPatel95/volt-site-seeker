      ...
      <div className="max-w-6xl mx-auto">
        <ScrollReveal direction="up">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--watt-navy)/0.1)] border border-[hsl(var(--watt-navy)/0.2)] mb-4">
              <Clock className="w-4 h-4 text-[hsl(var(--watt-navy))]" />
              <span className="text-sm font-medium text-[hsl(var(--watt-navy))]">Timeline</span>
            </div>
              ...
              <ScrollReveal key={event.year} direction={index % 2 === 0 ? 'left' : 'right'} delay={index * 0.05}>
                <div className={`relative flex items-center gap-4 md:gap-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                  ...
                  <div className={`flex-1 ml-12 md:ml-0 ${index % 2 === 0 ? 'md:text-right md:pr-8' : 'md:text-left md:pl-8'}`}>
                    <div className={`inline-block bg-card rounded-xl p-5 shadow-institutional border border-border ${index % 2 === 0 ? 'md:ml-auto' : ''}`}>
                      <div className="text-sm font-bold text-[hsl(var(--watt-bitcoin))] mb-1">{event.year}</div>
        ...
        <ScrollReveal direction="up" delay={0.5}>
          <div className="mt-12 bg-card rounded-2xl p-6 border border-border shadow-institutional">
            <div className="flex items-center gap-3 mb-3">
              <Pizza className="w-6 h-6 text-[hsl(var(--watt-bitcoin))]" />
              <h3 className="text-xl font-bold text-foreground">Bitcoin Pizza Day</h3>
            </div>

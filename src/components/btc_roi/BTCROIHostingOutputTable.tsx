...
export const BTCROIHostingOutputTable: React.FC<BTCROIHostingOutputTableProps> = ({ hostingResults }) => {
  ...
  return (
    <div className="space-y-4">
          ...
          {hostingResults.energyRateBreakdown && (
            <Card className="border-blue-200 bg-blue-50/30 w-full">
              ...
              <CardContent className="pt-0 space-y-3 sm:space-y-4 p-3 sm:p-6">
                ...
                {hostingResults.energyRateBreakdown.taxBreakdown && (
                  <div className="bg-white rounded-lg border p-3 sm:p-4 w-full overflow-hidden">
                    <h4 className="font-semibold text-gray-800 mb-3 text-xs sm:text-sm">Tax Breakdown</h4>
                    <div className="grid grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm">
                      <div className="flex justify-between items-center py-1">
                        <span className="text-gray-600">Sales Tax:</span>
                        <span className="font-semibold">{formatPercent(hostingResults.energyRateBreakdown.taxBreakdown.salesTaxRate)}</span>
                      </div>
                      <div className="flex justify-between items-center py-1">
                        <span className="text-gray-600">Utility Tax:</span>
                        <span className="font-semibold">{formatPercent(hostingResults.energyRateBreakdown.taxBreakdown.utilityTaxRate)}</span>
                      </div>
                      <div className="flex justify-between items-center py-1">
                        <span className="text-gray-600">Environmental:</span>
                        <span className="font-semibold">{formatPercent(hostingResults.energyRateBreakdown.taxBreakdown.environmentalFeeRate)}</span>
                      </div>
                      <div className="flex justify-between items-center py-1 font-bold">
                        <span className="text-gray-800">Total Tax Rate:</span>
                        <span className="text-gray-800">{formatPercent(hostingResults.energyRateBreakdown.taxBreakdown.totalTaxRate)}</span>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {hostingResults.taxAnalysis && (
                  <TableRow>
                    <TableCell className="font-medium text-xs sm:text-sm">Taxes & Fees</TableCell>
                    <TableCell className="text-right text-red-600 text-xs sm:text-sm break-words">-{formatCurrency(hostingResults.taxAnalysis.totalAnnualTaxes)}</TableCell>
                    <TableCell className="text-right text-red-600 text-xs sm:text-sm break-words">
                      -{formatCurrency(hostingResults.taxAnalysis.totalAnnualTaxes / hostingResults.totalEnergyUsageKWh)}
                    </TableCell>
                  </TableRow>
                )}
      ...
      {hostingResults.costAnalytics && hostingResults.competitiveAnalysis && hostingResults.taxAnalysis && (

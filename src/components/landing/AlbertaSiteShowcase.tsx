...
export const AlbertaSiteShowcase = () => {
  ...
  return <section className="relative py-12 sm:py-16 md:py-24 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-hidden">
      {/* Background pattern */}
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="relative group mb-6 sm:mb-8">
          <div className="relative aspect-[16/9] sm:aspect-[21/9] rounded-xl sm:rounded-2xl overflow-hidden border-2 border-electric-blue/30 shadow-2xl hover:border-electric-blue/50 transition-all duration-500">
            <img src={facilityImages[selectedImage].src} alt={facilityImages[selectedImage].label} className="w-full h-full object-cover transition-all duration-700" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-8">
          {facilityImages.map((image, index) => <button key={index} onClick={() => setSelectedImage(index)} className={`relative group aspect-video rounded-lg overflow-hidden border-2 transition-all duration-300 ${selectedImage === index ? 'border-electric-blue shadow-lg shadow-electric-blue/30 scale-105' : 'border-slate-800 hover:border-electric-blue/50'}`}>
              <img src={image.src} alt={image.label} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
              <div className={`absolute inset-0 transition-opacity duration-300 ${selectedImage === index ? 'bg-electric-blue/20' : 'bg-slate-950/40 group-hover:bg-slate-950/20'}`}></div>
